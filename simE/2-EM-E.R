...
for (a in 1:n.subcases) {
  for (b in 1:n.rep) {
    ...
    # compute loglik truth:
    #true.weights = matrix(0, K, 2)
    #true.weights[1:K1,1] = 1
    #true.weights[(K1+1):K, 2] = 1
    #temp = glm_estimator(ylist = ylist, weights = true.weights, 
    #                     design = design_sbm(v, block_vector, sparse = FALSE), 
    #                     model = 'sbm', family = 'binomial')
    #logl_true = temp$loglik
