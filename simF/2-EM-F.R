...
for (a in 1:n.subcases) {
  for (b in 1:n.rep) {
    ...
    if (a == 1) {
      time = system.time(
        sol <- foreach(i = 1:10) %dopar% EM_2comp(ylist, K = K, v = v,
                                                  start = start.array[,,i], block_vector = block_vector,
                                                  model = 'sbm', maxit = 100, epsilon = 1e-4)
      )      
    }
    if (a == 2) {
      time = system.time(
        sol <- foreach(i = 1:10) %dopar% EM_3comp(ylist, K = K, v = v,
                                                  start = start.array[,,i], block_vector = block_vector,
                                                  model = 'sbm', maxit = 100, epsilon = 1e-4)
      )      
    }
    if (a == 3) {
      time = system.time(
        sol <- foreach(i = 1:10) %dopar% EM_4comp(ylist, K = K, v = v,
                                                  start = start.array[,,i], block_vector = block_vector,
                                                  model = 'sbm', maxit = 100, epsilon = 1e-4)
      )      
    }
    if (a == 4) {
      time = system.time(
        sol <- foreach(i = 1:10) %dopar% EM_5comp(ylist, K = K, v = v,
                                                  start = start.array[,,i], block_vector = block_vector,
                                                  model = 'sbm', maxit = 100, epsilon = 1e-4)
      )      
    }
    if (a == 5) {
      time = system.time(
        sol <- foreach(i = 1:10) %dopar% EM_6comp(ylist, K = K, v = v,
                                                  start = start.array[,,i], block_vector = block_vector,
                                                  model = 'sbm', maxit = 100, epsilon = 1e-4)
      )      
    }
    if (a == 6) {
      time = system.time(
        sol <- foreach(i = 1:10) %dopar% EM_7comp(ylist, K = K, v = v,
                                                  start = start.array[,,i], block_vector = block_vector,
                                                  model = 'sbm', maxit = 100, epsilon = 1e-4)
      )      
    }
